        ...
        <ol className="select-text space-y-3">
          {messages.map((m) => (
            <li key={m.id} className={`flex flex-col ${m.direction === 'out' ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[85%] whitespace-pre-wrap [overflow-wrap:anywhere] rounded-2xl px-4 py-2.5 text-sm text-[var(--ink)] ${m.direction === 'out' ? 'bg-[var(--accent-soft)]' : 'border border-[var(--line)] bg-[var(--panel)]'}`}>
                {m.body}
              </div>
